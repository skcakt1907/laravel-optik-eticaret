...
<table class="table-a">
    <thead><tr><th></th><th>Ürün</th><th>Kategori</th><th>Fiyat</th><th>Stok</th><th>Durum</th><th></th></tr></thead>
    <tbody>
    @forelse($products as $p)
        <tr>
            <td><img src="{{ $p->image_url }}" class="thumb"></td>
            <td><strong>{{ $p->name }}</strong><br><small class="text-muted">{{ $p->sku }} @if($p->featured)· ⭐ Öne çıkan @endif</small></td>
            <td>{{ $p->category?->name ?? '—' }}</td>
            <td>{{ money($p->current_price) }} @if($p->on_sale)<br><small class="text-muted"><s>{{ money($p->price) }}</s></small>@endif</td>
            <td>{{ $p->stock }}</td>
            <td><span class="pill {{ $p->durum ? 'paid' : 'failed' }}">{{ $p->durum ? 'Aktif' : 'Pasif' }}</span></td>
            <td>
                <div class="d-flex gap-1">
                    <a href="{{ route('admin.products.edit', $p) }}" class="btn-a sec sm"><i class="bi bi-pencil"></i></a>
                    <form action="{{ route('admin.products.destroy', $p) }}" method="POST" onsubmit="return confirm('Silinsin mi?')">@csrf @method('DELETE')<button class="btn-a danger sm"><i class="bi bi-trash"></i></button></form>
                </div>
            </td>
        </tr>
    @empty
